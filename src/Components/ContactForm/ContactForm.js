...
class ContactForm extends React.Component {
  ...
  onChangeBehavior = (e) => {
    ...
    switch (name) {
      ...
      case 'surname':
        ...
        break;
      case 'email':
        errors.errEmail = validEmailRegex.test(value) ? '' : 'Wpisz poprawny adres email';
        break;
      case 'phoneNumber':
        errors.errPhoneNumber =
          value.length <= 0
            ? ''
            : (errors.errPhoneNumber =
                isNaN(value) || value.length < 9 || value.length > 9 ? 'Wpisz poprawny numer' : '');
        break;
      case 'message':
        errors.errMessage = value.length <= 1 ? 'Nie możesz wysłać pustej wiadomości' : '';
        break;
      default:
        break;
    }
  ...
  handleSubmit = (e) => {
    ...
    if (validateForm(this.state.errors)) {
      ...
      let tmeplateParams = {
        from_name:
          subject + ' od: ' + name + ' ' + surname + ' (' + email + ') przez formularz www SE',
        to_name: 'Bartek',
        message_html: subject + ':' + '<br>' + message,
        reply_to: name + ' ' + surname + '<br>' + email + '<br>' + phoneNumber
      };

      emailjs.send('gmail', 'template_quJaKsaU', tmeplateParams, 'user_2vc8gy0BAJr6Mjy0ZkG7D').then(
        (res) => {
          toString(res);
  ...
  render() {
    ...
    return (
      <form onSubmit={this.handleSubmit}>
        ...
        <button className='btn__submit' type='submit' name='submit' onClick={this.handleSubmit}>
          Wyślij wiadomość
        </button>
        ...
        {this.state.formSent ? (
          <div className='success__msg'>
            <div className='success__msg__content'>
              <h2>{name},</h2>
              <p>
                Dziękuję za kontakt. Najprawdopodobniej właśnie dostałem od Ciebie wiadomość. Zwykle
                odpowiadam w przeciągu kilku godzin, więc do usłyszenia.
              </p>
